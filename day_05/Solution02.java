...
public class Solution02 {
  ...
  private static ArrayList<String> correctUpdate(HashSet<String> orderSet, ArrayList<String> update) {
    for (int i = 0; i < update.size() - 1; i++) {
      for (int j = i + 1; j < update.size(); j++) {
        String firstPage = update.get(i);
        String secondPage = update.get(j);
        ...
        if (orderSet.contains(String.format("%s|%s", secondPage, firstPage))) {
          update.set(i, secondPage);
          update.set(j, firstPage);
          j = i + 1; // Reset the check again as j is now a new page
        }
      }
    }
